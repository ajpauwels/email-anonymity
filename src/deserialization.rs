...
pub fn string_is_ecc_public_key<'de, D>(deserializer: D) -> Result<PublicKey, D::Error>
where
    D: Deserializer<'de>,
{
    ...
    if key_string.is_empty() {
        ...
        let pk = PublicKey::from_slice(key_bytes.as_ref())
...
// pub fn string_is_ecc_secret_key<'de, D>(deserializer: D) -> Result<SecretKey, D::Error>
// where
//     D: Deserializer<'de>,
// {
//     let key_string = String::deserialize(deserializer)?;
...
//             .decode(key_string)
//             .map_err(|_| de::Error::custom(CryptoDeserializationError::InvalidBase64))?;
//         let sk = SecretKey::from_slice(key_bytes.as_ref())
//             .ok_or(de::Error::custom(CryptoDeserializationError::BadLength))?;
//         Ok(sk)
//     }
// }
